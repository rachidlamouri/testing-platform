...
import { mutateGraphLikeElementListOrder } from '../../programmable-units/type-script-file-relationships/graph-element/mutateGraphLikeElementListOrder';
import {
...
export const getDirectedGraphFromGraphElementGroup = buildEstinant({
  ...
  .onPinbe((graphElementGroup) => {
    ...
    const [directedGraph] = allRootGraphList;

    const graphById = new Map(
      [directedGraph, ...allSubgraphList.map(({ subgraph1 }) => subgraph1)].map(
        (graph) => {
    ...
    mutateGraphLikeElementListOrder(directedGraph);

    return {
      [PROGRAM_ERROR_GEPP]: parallelErrorList,
      [DIRECTED_GRAPH_GEPP]: [directedGraph],
    };
  })
  .assemble();
