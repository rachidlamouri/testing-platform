...
export const engineProgramRendererWortinator =
  buildWortinator<EngineProgramTreeNodeVoictent>({
    ...
    pinbe: ({ grition: input }) => {
      ...
      const identifiableEntityNodeMap = new Map<
        string,
        IdentifiableEntityNode
      >();
      allPrerenderNodes
...
`;
      ...
      const fileName = `${input.programName}.md`;

      fileUtilities.writeOutputFile(fileName, markdown);
    },
  });
