...
export type DirectoryCreatorResult<TRoot extends Root> = TRoot extends typeof USE_CWD ? RootDirectoryHelper : NestedDirectoryHelper

export const createDirectory = <TRoot extends Root>({
  root,
  relativePath,
}: DirectoryCreatorParameter<TRoot>):DirectoryCreatorResult<TRoot>  => {
  const directoryPath = root !== USE_CWD ? `${root.directoryPath}/${relativePath}`: relativePath
